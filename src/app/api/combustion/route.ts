...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const tipoCombustibleId =
      searchParams.get("tipoCombustibleId") ?? undefined;
    ...
    const whereOptions = {
      ...
      tipoCombustible_id: tipoCombustibleId
        ? parseInt(tipoCombustibleId)
        : undefined,
    ...
    const totalRecords = await prisma.combustible.count({
      where: whereOptions,
    });
