...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (perPage > 0) {
      const totalRecords = await prisma.factorConversionSEIN.count({
        where: whereOptions,
      });
    ...
    return NextResponse.json(
      factoresConversionSEIN.map(formatfactorEmisionSEIN)
    );
  } catch (error) {
    console.error("Error buscando Factores de Conversión SEIN", error);
    return NextResponse.json(
      { message: "Error buscando Factores de Conversión SEIN" },
      { status: 500 }
    );
  }
}
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    return NextResponse.json(
      { message: "Error creando el Factor de Conversión SEIN" },
      { status: 500 }
    );
  }
}
