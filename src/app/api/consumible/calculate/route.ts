...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const toValue =
      yearToId && mesToId ? Number(yearTo) * 100 + mesToId : undefined;
    ...
    const formattedConsumibleCalculos: any[] = consumibleCalculos.map(
      (consumibleCalculo, index: number) => {
  ...
  try {
    ...
    for (const tipoConsumible of tiposConsumible) {
      ...
      for (const period of allPeriodsBetweenYears) {
        ...
        const factorTipoConsumible =
          await prisma.factorTipoConsumible.findFirst({
            where: { anioId, tipoConsumibleId: tipoConsumible.id },
          });

        if (!factorTipoConsumible)
          return new NextResponse(
            `Agregue el factor de tipo de consumible de ${tipoConsumible.nombre} para el año ${period.anio}`,
            { status: 404 }
          );
        ...
        const pesoTotalTipoConsumible: number = consumibles.reduce(
          (acc, consumible) => {
        ...
        const totalGEIConsumible: number =
          pesoTotalTipoConsumible * factorTipoConsumible.factor;
