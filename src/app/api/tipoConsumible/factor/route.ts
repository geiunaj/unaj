...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const whereOptions = {
      tipoConsumibleId: tipoConsumibleId
        ? parseInt(tipoConsumibleId)
        : undefined,
      anioId: anioId,
    };
    ...
    const tiposConsumibleFactor = await prisma.factorTipoConsumible.findMany({
      ...
      orderBy: [{ tipoConsumibleId: "desc" }, { anio: { nombre: "desc" } }],
      ...(perPage > 0 ? { skip: (page - 1) * perPage, take: perPage } : {}),
    });

    if (perPage > 0) {
      const totalRecords = await prisma.factorTipoConsumible.count({
        where: whereOptions,
      });
    ...
    return NextResponse.json(
      tiposConsumibleFactor.map(formatTipoConsumibleFactor)
    );
  } catch (error) {
    console.error("Error buscando Factores Tipos Consumible", error);
    return new NextResponse("Error buscando Factores Tipos Consumible", {
      status: 500,
    });
  }
}
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    return new NextResponse("Error creando el Factor Tipo de Consumible", {
      status: 500,
    });
  }
}
