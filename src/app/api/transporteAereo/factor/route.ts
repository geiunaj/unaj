...
import { TransporteAereoFactorRequest } from "@/components/transporte-aereo-factor/services/transporteAereoFactor.interface";
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const factoresTransporte =
      await prisma.factorEmisionTransporteAereo.findMany({
        where: whereOptions,
        include: { anio: true },
    ...
    if (perPage > 0) {
      const totalRecords = await prisma.factorEmisionTransporteAereo.count({
        where: whereOptions,
      });
    ...
    return NextResponse.json(
      factoresTransporte.map(formatTransporteAereoFactor)
    );
  } catch (error) {
    console.error("Error buscando Factores de Transporte Aereo", error);
    return new NextResponse("Error buscando Factores de Transporte Aereo", {
      status: 500,
    });
  }
}
  ...
  try {
    ...
    if (factorExistente) {
      return new NextResponse(
        "Ya existe un factor de Transporte Aereo para este año",
        { status: 400 }
      );
    }
    const factorEmisionTransporteAereo =
    ...
    return new NextResponse(
      "Error creando el Factor de Transporte Aereo SEIN",
      { status: 500 }
    );
  }
}
