...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const whereOptions = tipoCombustibleId
      ? { tipoCombustible_id: parseInt(tipoCombustibleId) }
      : {};
    ...
    if (perPage > 0) {
      const totalRecords = await prisma.tipoCombustibleFactor.count({
        where: whereOptions,
      });
    ...
    return NextResponse.json(
      tiposCombustibleFactor.map(formatTipoCombustibleFactor)
    );
  } catch (error) {
    console.error("Error buscando Factores Tipos Combustible", error);
    return new NextResponse("Error buscando Factores Tipos Combustible", {
      status: 500,
    });
  }
}
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    return new NextResponse("Error creando el Factor Tipo de Combustible", {
      status: 500,
    });
  }
}
