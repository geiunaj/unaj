  ...
  try {
    ...
    if (!TransporteCasaTrabajo) {
      return NextResponse.json(
        { message: "Transporte Casa Trabajo no encontrado" },
        { status: 404 }
      );
    }
    return NextResponse.json(
      formatTransporteCasaTrabajo(TransporteCasaTrabajo)
    );
  } catch (error) {
    console.error("Error buscando Transporte Casa Trabajo", error);
    return NextResponse.json(
      { message: "Error buscando Transporte Casa Trabajo" },
      { status: 500 }
    );
  }
}
...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json("Error actualizando Transporte Casa Trabajo", {
      status: 500,
    });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { message: "Error eliminando Transporte Casa Trabajo" },
      { status: 500 }
    );
  }
}
