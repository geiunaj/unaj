...
import { TransporteTerrestreFactorRequest } from "@/components/transporte-terrestre-factor/services/transporteTerrestreFactor.interface";
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const factoresTransporte =
      await prisma.factorEmisionTransporteTerrestre.findMany({
        where: whereOptions,
        include: { anio: true },
    ...
    if (perPage > 0) {
      const totalRecords = await prisma.factorEmisionTransporteTerrestre.count({
        where: whereOptions,
      });
    ...
    return NextResponse.json(
      factoresTransporte.map(formatTransporteTerrestreFactor)
    );
  } catch (error) {
    console.error("Error buscando Factores de Transporte Terrestre", error);
    return new NextResponse("Error buscando Factores de Transporte Terrestre", {
      status: 500,
    });
  }
}
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const factorExistente =
      await prisma.factorEmisionTransporteTerrestre.findFirst({
        where: {
          anio_id: body.anioId,
        },
      });
    if (factorExistente) {
      return new NextResponse(
        "Ya existe un factor de Transporte Terrestre para este año",
        { status: 400 }
      );
    }
    const factorEmisionTransporteTerrestre =
    ...
    console.error(
      "Error creando el Factor de Transporte Terrestre SEIN",
      error
    );
    return new NextResponse(
      "Error creando el Factor de Transporte Terrestre SEIN",
