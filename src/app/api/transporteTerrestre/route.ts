...
} from "@/components/transporteTerrestre/service/transporteTerrestre.interface";
...
export async function GET(req: NextRequest): Promise<NextResponse> {
    try {
        ...
        const totalRecords = await prisma.transporteTerrestre.count({where: whereOptions});
        ...
        return new NextResponse("Error finding transporteTerrestre", {status: 500});
    }
}
...
export async function POST(req: NextRequest): Promise<NextResponse> {
    try {
        ...
        const formattedTransporteTerrestre = formatTransporteTerrestre(transporteTerrestre);
        ...
        return new NextResponse("Error creando Transporte Terrestre", {status: 500});
    }
}
