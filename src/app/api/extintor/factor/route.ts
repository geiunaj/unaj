...
import { ExtintorFactorRequest } from "@/components/extintorFactor/services/extintorFactor.interface";
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (perPage > 0) {
      const totalRecords = await prisma.factorEmisionExtintor.count({
        where: whereOptions,
      });
    ...
    return NextResponse.json(
      { message: "Error buscando Factores de Extintor" },
      { status: 500 }
    );
  }
}
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (factorExistente) {
      return NextResponse.json(
        { message: "Ya existe un factor de Extintor para este año" },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { message: "Error creando el Factor de Extintor" },
      { status: 500 }
    );
  }
}
