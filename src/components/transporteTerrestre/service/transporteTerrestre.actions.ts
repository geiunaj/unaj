...
import {TransporteTerrestreCollection, TransporteTerrestreRequest} from "./transporteTerrestre.interface";
...
export async function getTransporteTerrestre(
    ...
    const {data} = await api.get<TransporteTerrestreCollection>("/api/transporteTerrestre", config);
    return data;
}
...
export async function getTransporteTerrestreById(id: number) {
...
export async function deleteTransporteTerrestre(id: number): Promise<AxiosResponse<Response>> {
