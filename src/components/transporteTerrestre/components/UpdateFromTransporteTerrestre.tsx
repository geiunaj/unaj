...
import {TransporteTerrestreRequest, UpdateTransporteTerrestreProps} from "../service/transporteTerrestre.interface";
...
export function UpdateFormTransporteTerrestre({id, onClose}: UpdateTransporteTerrestreProps) {
    ...
    const loadForm = useCallback(() => {
        if (transporteTerrestres.data) {
            form.reset({
                numeroPasajeros: transporteTerrestres.data.numeroPasajeros ?? 0,
                origen: transporteTerrestres.data.origen,
                destino: transporteTerrestres.data.destino,
                isIdaVuelta: transporteTerrestres.data.isIdaVuelta,
                fechaSalida: transporteTerrestres.data.fechaSalida ? new Date(transporteTerrestres.data.fechaSalida) : undefined,
                fechaRegreso: transporteTerrestres.data.fechaRegreso ? new Date(transporteTerrestres.data.fechaRegreso) : undefined,
                distanciaTramo: transporteTerrestres.data.distanciaTramo ?? undefined,
                kmRecorrido: transporteTerrestres.data.kmRecorrido ?? undefined,
                anio: transporteTerrestres.data.anio_id.toString(),
                sede: transporteTerrestres.data.sede_id.toString(),
                mes: transporteTerrestres.data.mes_id.toString(),
            });
        }
    ...
    const onSubmit = async (data: z.infer<typeof TransporteTerrestreSchema>) => {
        ...
        try {
            const response = await updateTransporteTerrestre(id, TransporteTerrestreRequest);
            onClose();
