...
export default function Header({ urlLogo, urlLogoDark }: HeaderProps) {
  ...
  const redirectToApp = () => {
    window.open(
      "https://drive.google.com/uc?export=download&id=1NcLQfdB8vABr7ZXh4LVrxd13ykIuSPID"
    );
  };
  ...
  return (
    <header className="flex h-14 justify-between md:justify-center items-center sm:gap-4 border-b bg-muted/40 px-4 lg:h-[60px] lg:px-6">
      <Sheet>
        ...
        <SheetContent side="left" className="flex flex-col p-4">
              ...
              return item.items ? (
                ...
                >
                  <AccordionItem className="border-0" value={item.title}>
                    <AccordionTrigger className="w-full px-4 py-2 justify-between items-center text-muted-foreground hover:text-primary hover:no-underline">
          ...
          <div className="grid items-start px-2 text-xs font-medium overflow-auto p-8">
            <Button onClick={() => redirectToApp()} className="text-white">
              <Smartphone className="mr-2 h-3.5 w-3.5" />
              <p className="ml-3 text-xs">Descarga la APP</p>
            </Button>
          </div>
        </SheetContent>
      </Sheet>
      ...
      <div className="max-w-screen-xl flex items-center justify-between w-full">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem className="text-xs" onClick={handleAccount}>
              Cuenta
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuGroup>
