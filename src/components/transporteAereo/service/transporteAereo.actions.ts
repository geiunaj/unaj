...
import {TransporteAereoCollection, TransporteAereoRequest} from "./transporteAereo.interface";
...
export async function getTransporteAereo(
    ...
    const {data} = await api.get<TransporteAereoCollection>("/api/transporteAereo", config);
    return data;
}
...
export async function deleteTransporteAereo(id: number): Promise<AxiosResponse<Response>> {
