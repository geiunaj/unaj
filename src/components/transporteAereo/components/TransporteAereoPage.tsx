...
import {
  useTransporteAereo,
  useTransporteAereoReport,
} from "../lib/transporteAereo.hook";
import {
  errorToast,
  formatPeriod,
  successToast,
...
export default function TransporteAereoPage() {
  ...
  const handleClickReport = async (period: ReportRequest) => {
    ...
    await GenerateReport(
      transporteAereoReport.data!.data,
      columns,
      formatPeriod(period),
      "REPORTE DE TAXIS CONTRATADOS",
      "TransporteAereos Contratados"
    );
  };
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="font-Manrope text-sm font-bold text-center">
                DISTANCIA
                <br /> TRAMO <span className="text-xs">[km]</span>
              </TableHead>
              <TableHead className="font-Manrope text-sm font-bold text-center">
                KM <br />
                RECORRIDO
