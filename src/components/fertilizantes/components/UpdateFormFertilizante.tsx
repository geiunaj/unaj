...
export function UpdateFormFertilizantes({
  ...
  async function loadForm(id: number) {
    try {
      const fertilizante = await showFertiliante(id);
      form.reset({
        clase: fertilizante.clase || "",
      ...
      console.error("Error loading fertilizante form:", error);
    }
  }
  ...
  useEffect(() => {
    ...
    loadForm(id);
  }, [loadSedes, loadTiposFertilizante, loadAnios, id]);
